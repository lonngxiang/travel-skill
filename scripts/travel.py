...
def load_config() -> Dict[str, Any]:
    """
    加载配置，优先使用环境变量 TRAVEL_MCP_URL，
    其次读取 SKILL 目录下的 config.json。
    """
    base_url = os.environ.get("TRAVEL_MCP_URL", "").strip()
    if base_url:
...
def print_usage():
    ...
    print(f"""
使用说明：
    python3 scripts/travel.py <method> [params]
...
配置方式（二选一）：
    1. 环境变量：export TRAVEL_MCP_URL=http://<host>:<port>/mcp
    2. 配置文件：cp config.example.json config.json  # 然后填写 baseUrl

调用示例（在技能目录下执行）：
    cd {skill_dir}
    python3 scripts/travel.py tools/list
    python3 scripts/travel.py tools/call '{{"name": "check_hotel", "arguments": {{"cityname": "北京", "datein": "2026-05-01"}}}}'
    python3 scripts/travel.py tools/call '{{"name": "check_flight", "arguments": {{"ori_cityname": "北京", "des_cityname": "上海", "date": "2026-05-01"}}}}'
    python3 scripts/travel.py tools/call '{{"name": "check_train", "arguments": {{"ori_cityname": "北京", "des_cityname": "上海", "date": "2026-05-01"}}}}'
""")
...
def main():
    ...
    try:
        ...
        if not base_url:
            print("[错误] 配置错误: 未找到 baseUrl（检查 config.json 或 TRAVEL_MCP_URL 环境变量）")
            sys.exit(1)
