...
基于天翼出行（118114出行）MCP 服务的智能出行技能，支持机票、火车票、酒店实时查询，航班动态追踪，订单管理，景点评价，天气查询等功能。
...
```bash
git clone https://github.com/lonngxiang/tianyi-travel-mcp.git
cd tianyi-travel-mcp
```
...
```bash
# 查酒店
python3 scripts/tianyi_travel.py tools/call '{"name": "check_hotel", "arguments": {"cityname": "上海", "datein": "2026-05-01"}}'

# 查机票
python3 scripts/tianyi_travel.py tools/call '{"name": "check_flight", "arguments": {"ori_cityname": "北京", "des_cityname": "上海", "date": "2026-05-01"}}'

# 查高铁
python3 scripts/tianyi_travel.py tools/call '{"name": "check_train", "arguments": {"ori_cityname": "北京", "des_cityname": "上海", "date": "2026-05-01", "trainType": "G"}}'
```
...
设置环境变量或在 OpenClaw 技能配置中填写 `TIANYI_BASE_URL`。
...
# 服务启动后设置：
export TIANYI_BASE_URL=http://127.0.0.1:7020/mcp
```
...
- 推荐使用环境变量 `TIANYI_BASE_URL` 管理服务地址
- 无需 Token，无鉴权配置
